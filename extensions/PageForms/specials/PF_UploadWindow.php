...
END;
    ...
    $output .= <<<END
    try{parent.change_image_thumb("{$this->mInputID}");}catch(err){console.log(err);}
    try{parent.document.getElementsByTagName('body')[0].classList.remove('compensate-for-scrollbar');}catch(err){console.log(err);}
    try{parent.jQuery.fancybox.close();}catch(err){parent.document.getElementsByClassName('fancybox-container')[0].parentNode.removeChild(parent.document.getElementsByClassName('fancybox-container')[0]);}
	</script>
