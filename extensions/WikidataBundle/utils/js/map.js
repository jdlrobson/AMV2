  ...
  $.get($url, function(result, status) {
    ...
    for (var key in artworks_map) {
      features.push(new ol.Feature({
        geometry: new ol.geom.Point(ol.proj.transform([artworks_map[key].longitude, artworks_map[key].latitude], "EPSG:4326", "EPSG:3857")),
        title: artworks_map[key].title,
        artist: artworks_map[key].creator.join(", "),
        id: artworks_map[key].wikidata,
        type: artworks_map[key].nature,
        date: artworks_map[key].date,
        article: artworks_map[key].article,
        image: artworks_map[key].image
      }));}
